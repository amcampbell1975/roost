...
handler = RotatingFileHandler('/tmp/midea_mqtt_control.log.txt', maxBytes=100000, backupCount=3) 
...
def on_connect(client, userdata, flags, rc):
    ...
    client.subscribe("midea/set_temperature")
    client.subscribe("midea/set_power")
...
def on_message(client, userdata, msg):
    ...
    if msg.topic=="midea/set_temperature":
       p=str(msg.payload)
       ...
       if temperature<20 :
            temperature=20
            Modbus_HW(enable=False, temperature=temperature)
       else:
           if temperature>60:
                temperature=60
           Modbus_HW(enable=True, temperature=temperature)
       logger.info("Setting temperature     = %d" %(temperature))
