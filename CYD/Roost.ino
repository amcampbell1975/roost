...
void AddNewHotWaterTop(float t ){
  for (int i = 0; i <320-1 ; i++){
    temps_top[i]=temps_top[i+1];
    }
  temps_top[319]=t;
}
void AddNewHotWaterUpper(float t ){
  for (int i = 0; i <320-1 ; i++){
    temps_upp[i]=temps_upp[i+1];
    }
  temps_upp[319]=t;
}
void AddNewHotWaterLower(float t ){
  for (int i = 0; i <320-1 ; i++){
    temps_low[i]=temps_low[i+1];
    }
  temps_low[319]=t;
}
void AddNewHotWaterBottom(float t ){
  for (int i = 0; i <320-1 ; i++){
    temps_bot[i]=temps_bot[i+1];
    }
  temps_bot[319]=t;
}
...
void HotGraph(void){//float temperatures[10]){
  ...
  for(int i=0;i<320;i++){
    //Serial.printf("%d - %f\n",i,temps[i]);
    float h= map(temps_top[i],0,100,G_HEIGHT,0  );
    //tft.drawFastVLine(i , 0,      h, TFT_YELLOW);
    const int index=limit(map(temps_top[i],10,65,19,0),0,19);
  ...
  for(int i=0;i<320;i++){
    ...
    h= map(temps_upp[i],0,100,G_HEIGHT,0  );
    tft.drawPixel(i,h , TFT_WHITE); 
    h= map(temps_low[i],0,100,G_HEIGHT,0  );
    tft.drawPixel(i,h , TFT_WHITE); 
    h= map(temps_bot[i],0,100,G_HEIGHT,0  );
    tft.drawPixel(i,h , TFT_WHITE); 
  }
